...
public class PoubelleVisualManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < fruits.Count; i++)
        {
            ...
            fruits[i].transform.position = Vector3.Lerp(basePos[i], Vector3.Lerp(midlePosOffseted, endPos, destinationCurve.Evaluate(fruitTimers[i])), (_ejectFruits? speedCurveDropping : speedCurve).Evaluate(fruitTimers[i]));//deplacement fruit
    ...
    IEnumerator RandomDelayedFruits()
    {
        ...
        for (ejectedFruits = 0; ejectedFruits < storedFruits.Count; ejectedFruits++)
        {
            ...
            InitializeMovement(storedFruits[ejectedFruits]);
