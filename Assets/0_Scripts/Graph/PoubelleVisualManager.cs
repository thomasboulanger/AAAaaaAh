...
public class PoubelleVisualManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        foreach (Transform item in toRemoveFromLists)
        {
            ...
            fruits.RemoveAt(index); fruitTimers.RemoveAt(index); basePos.RemoveAt(index); hasBouped.RemoveAt(index); middlePosOffsets.RemoveAt(index); hasBoupedMonster.RemoveAt(index); //on suprime tt les fruits d�truits cette frame
